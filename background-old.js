...
/* function handleData(data) {
    return new Promise((resolve, reject) => {
        try {
            ...
            resolve(submissionData); // Resolve the promise with submissionData
        } catch (error) {
...
function storeJotformData(data) {
    console.log("storeJotformData", data);
...
/* function sendGmailDraft(to, subject, body) {
    return new Promise((resolve, reject) => {
        authenticateGoogle(function (token) {
            if (!token) {
                console.error("Authentication failed: No token received.");
...
/* function createInvoice(data) {
    return new Promise((resolve, reject) => {
        ...
        if (data.additionalBoxedFurClipping) {
            products.push({
                name: `Paw Impression Box x${data.additionalBoxedFurClipping}`,
        ...
        if (data.additionalFurClipping) {
            products.push({
                name: `Fur Clipping x${data.additionalFurClipping}`,
        ...
        if (data.additionalNosePrint) {
            products.push({
                name: `Framed Nose Print x${data.additionalNosePrint}`,
        ...
        if (data.additonalPawPrint) {
            products.push({
                name: `Framed Paw Print x${data.additonalPawPrint}`,
...
function createInvoice(data) {
    ...
    return new Promise((resolve, reject) => {
        ...
        console.log(myData.invoiceTemplateId);
